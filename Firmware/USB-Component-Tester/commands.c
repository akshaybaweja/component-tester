...
uint8_t RunCommand(uint8_t ID)
{
  ...
  switch (ID)
  {
    ...
    case CMD_PROBE_ID:             /* return firmware version */
      Display_EEString(probe_colors_str);         /* send firmware version & newline */
      break;
    ...
    case CMD_OFF:             /* power off */
      ...
      Display_EEString_NL(Cmd_OK_str);          /* send: OK */
      break;
  ...
  {
    Display_EEString_NL(Cmd_ERR_str);      /* send: ERR */
  }
  else if (Flag == SIGNAL_NA)      /* n/a */
  {
    Display_EEString_NL(Cmd_NA_str);       /* send: N/A */
  }
